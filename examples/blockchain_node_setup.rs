...
impl BlockchainNode {
    ...
    async fn new(config: BlockchainNodeConfig) -> Result<Self> {
        ...
        let mut client_config = StreamlinedConfig::default();
        client_config.max_channels = config.validator_peers.len() + 10; // Extra capacity
    ...
    async fn establish_validator_network(&mut self) -> Result<()> {
        ...
        let results = self.secure_client.establish_blockchain_validator_network(
            self.config.validator_peers.clone(),
            self.config.topology.clone(),
            Some(self.config.channel_config.clone()),
        ).await?;
            ...
            match self.secure_client.send_secure_message(target_validator, message).await {
                ...
                Err(e) => {
                    ...
                    if attempts < max_attempts {
                        println!("⚠️  Message send attempt {} failed: {}, retrying...", attempts, e);
                        sleep(Duration::from_millis(500)).await;
                    } else {
                        self.message_stats.failed_deliveries += 1;
                        println!("❌ Message send failed after {} attempts: {}", max_attempts, e);
                        return Err(e);
                    }
                }
            }
    ...
    async fn flood_message_to_validators(&mut self, message: &[u8]) -> Result<Vec<SecureMessage>> {
        ...
        let peer_ids: Vec<String> = self.config.validator_peers.iter().cloned().collect();
...
async fn demonstrate_blockchain_node_setup() -> Result<()> {
    ...
    let test_messages = vec![
        "BLOCKCHAIN_CONSENSUS:PROPOSE_BLOCK_12345",
    ...
    // Avoid borrow checker issue by cloning peer IDs
    let peer_ids: Vec<String> = blockchain_node.config.validator_peers.iter().cloned().collect();
...
async fn demonstrate_continuous_operation() -> Result<()> {
    ...
    // Avoid borrow checker issue by cloning peer IDs
    let peer_ids: Vec<String> = continuous_node.config.validator_peers.iter().cloned().collect();
    for _ in 0..5 {
        interval.tick().await;
        operation_count += 1;
        // Health check
        ...
        if is_healthy {
            ...
            match continuous_node.send_blockchain_message(target, periodic_message.as_bytes()).await {
                Ok(_) => println!("   ✅ Cycle {}: Message sent to {}", operation_count, target),
                Err(e) => println!("   ❌ Cycle {}: Message failed - {}", operation_count, e),
            }
        }
