...
async fn test_initial_state_fidelity(quantum_core: &mut QuantumCore) -> Result<()> {
    ...
    println!("  Manual calculation: Σ|ψᵢ|² = {:.10}", manual_fidelity);
    println!("  System calculation: {:.10}", system_fidelity);
...
async fn test_single_qubit_gates(quantum_core: &mut QuantumCore) -> Result<()> {
    ...
    for (name, gate) in gates {
        ...
        println!("{} gate on qubit 0:", name);
        println!("  Initial fidelity: {:.10}", initial_fidelity);
        println!("  Final fidelity: {:.10}", final_fidelity);
        println!("  Manual check: {:.10}", manual_fidelity);
        println!("  ✅ Unitary preservation: {}", (final_fidelity - 1.0).abs() < 1e-10);
        
        assert!((final_fidelity - 1.0).abs() < 1e-10, "{} gate broke unitarity", name);
        assert!((manual_fidelity - final_fidelity).abs() < 1e-10, "{} fidelity calculation error", name);
    }
...
async fn test_two_qubit_gates(quantum_core: &mut QuantumCore) -> Result<()> {
    ...
    println!("  After Hadamard: {:.10}", after_h_fidelity);
    println!("  After CNOT: {:.10}", final_fidelity);
    println!("  Final amplitudes: {:?}", amplitudes);
    println!("  Manual calculation: {:.10}", manual_fidelity);
...
async fn test_superposition_fidelity(quantum_core: &mut QuantumCore, qrng: &mut QRNG) -> Result<()> {
    ...
    println!("  Expected amplitude: {:.10}", expected_amplitude);
    println!("  Amplitude variance: {:.2e}", amplitude_variance);
    println!("  System fidelity: {:.10}", system_fidelity);
    println!("  Manual fidelity: {:.10}", manual_fidelity);
...
async fn test_measurement_fidelity(quantum_core: &mut QuantumCore, qrng: &mut QRNG) -> Result<()> {
    ...
    println!("  Pre-measurement fidelity: {:.10}", pre_measurement_fidelity);
    println!("  Measurement result: {:?}", measurement_result);
    println!("  Post-measurement fidelity: {:.10}", post_measurement_fidelity);
    println!("  Collapsed amplitudes: {:?}", amplitudes);
    println!("  Manual verification: {:.10}", manual_fidelity);
...
async fn test_mathematical_principles(quantum_core: &mut QuantumCore) -> Result<()> {
    ...
    println!("  Σᵢ |ψᵢ|² = {:.15}", born_rule_sum);
    ...
    println!("  After Hadamard: Σᵢ |ψᵢ|² = {:.15}", born_rule_sum_h);
    ...
    println!("  Expected amplitude: 1/√2 = {:.15}", expected_amp);
...
async fn test_physics_validation(quantum_core: &mut QuantumCore, qrng: &mut QRNG) -> Result<()> {
...
async fn test_performance_consistency(quantum_core: &mut QuantumCore, qrng: &mut QRNG) -> Result<()> {
    let iterations = 1000;
    ...
    println!("  Average fidelity: {:.15}", avg_fidelity);
    println!("  Fidelity std dev: {:.2e}", fidelity_stddev);
    println!("  Average timing: {:.0} ns", avg_timing);
    println!("  Timing range: {:.0} - {:.0} ns", min_timing, max_timing);
