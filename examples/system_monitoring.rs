...
async fn monitor_performance_metrics() -> Result<()> {
    ...
    println!("   Component time: {}ms", total_component_time);
    println!("   System overhead: {}ms", overhead_ms);
    ...
    println!("   Target time: {}ms", target_total_ms);
    println!(
...
async fn run_health_checks() -> Result<()> {
    ...
    println!("     System status query: {}ms", response_time);
...
async fn simulate_load_monitoring() -> Result<()> {
    ...
    let mut config = StreamlinedConfig::default();
    config.max_channels = 20; // Increased for load testing
    config.enable_monitoring = true;
    ...
    if !establishment_times.is_empty() {
        ...
        println!("   Connection time range: {}ms - {}ms", min_time, max_time);
        println!(
