...
async fn demonstrate_channel_establishment_optimization() -> Result<()> {
    ...
    println!("   • Estimated Sequential Time: {}ms", estimated_sequential_time);
...
async fn demonstrate_blockchain_topologies() -> Result<()> {
    ...
    for (topology, description) in topologies {
        ...
        match client.establish_blockchain_validator_network(
            validators.clone(), 
            topology, 
            Some(config)
        ).await {
...
async fn demonstrate_retry_optimization() -> Result<()> {
    ...
    for (config, strategy_name) in retry_strategies {
        ...
        if retry_recoveries > 0 {
            println!("   💡 Recovered {} failures through retries", retry_recoveries);
        }
    }
